...
```
先来说下多出来的这个方法： _xx_block_copy_0 和 _xx_block_dispose_0，为了方便阅读，下面会把test_init简写为_xx
...
_Block_object_assign函数会自动根据_xx_block_impl_0结构体内部的“a”是什么类型的指针，对“a”对象产生强引用或者弱引用。可以理解为_Block_object_assign函数内部会对“a”进行引用计数器的操作，如果_xx_block_impl_0结构体内“a”指针是__strong类型，则为强引用，引用计数+1，如果_xx_block_impl_0结构体内“a”指针是__weak类型，则为弱引用，引用计数不变。
