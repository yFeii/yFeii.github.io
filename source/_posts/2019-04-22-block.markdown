...
\_Block_object_assign函数会自动根据\_xx_block_impl_0结构体内部的“a”是什么类型的指针，对“a”对象产生强引用或者弱引用。可以理解为_Block_object_assign函数内部会对“a”进行引用计数器的操作，如果\_xx_block_impl_0结构体内“a”指针是 \__strong 类型，则为强引用，引用计数+1，如果\_xx_block_impl_0结构体内“a”指针是 \__weak 类型，则为弱引用，引用计数不变。
